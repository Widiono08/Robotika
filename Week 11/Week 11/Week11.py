...
# Inisialisasi lidar
lidar = robot.getDevice('lidar')  # Pastikan nama perangkat lidar sesuai
lidar.enable(TIME_STEP)
lidar.enablePointCloud()

# Inisialisasi sensor jarak (ultrasonic)
us = [robot.getDevice('us0'), robot.getDevice('us1')]  # Pastikan nama perangkat sensor sesuai
for sensor in us:
    sensor.enable(TIME_STEP)

# Inisialisasi motor
left_motor = robot.getDevice('left wheel motor')  # Pastikan nama perangkat motor sesuai
right_motor = robot.getDevice('right wheel motor')  # Pastikan nama perangkat motor sesuai
...
def compute_speeds(us_values):
    ...
    for i in range(2):
        for k in range(2):
            speed[i] += us_values[k] * coefficients[i][k]  # Perbaiki perkalian koefisien
    return speed
